...
[SWS_CM_10218] Scope of length field value for structs
「  
シリアル化SOME/IPネットワークバインディングは、シリアル化された構造体のサイズ（バイト単位）を（長さフィールドのサイズなしで）構造体の長さフィールドに書き込みます。
」  

[SWS_CM_10219] Length greater than expected struct length 
「  
長さが構造体の予想される長さ（データ型定義で指定されている）よりも大きい場合、SOME / IPネットワークバインディングの逆シリアル化は、予想されるデータのみを解釈し、予期しないものをスキップします。
」  
...
[SWS_CM_10222] Setting the size of the length field for arrays 
「  
シリアル化SOME/IPネットワークバインディングは、シリアル化された配列のサイズ（バイト単位）を（長さフィールドのサイズなしで）長さフィールドに書き込みます。
」  
動的な長さの配列のレイアウトを7.31と図7.32に示します。ここで、L_1とL_2はバイト単位の長さを示します。 1次元および多次元の動的長さ配列のシリアル化については、次の2つのサブチャプターで説明します。　　
一次元  
1次元配列には、同じタイプの要素が多数含まれています。  
![](./image/Figure7_32.png)  
...
「  
シリアル化SOME/IPネットワークバインディングは、シリアル化されたバリアントのサイズ（バイト単位）（パディングバイトを含むが、長さフィールドとタイプフィールドのサイズを含まない）をバリアントの長さフィールドに書き込みます。この要件は、拡張可能な構造体とメソッドのシリアル化には適用されません。
」  

[SWS_CM_10227] Length greater than expected Variant length  
「  
長さがバリアントの予想される長さよりも大きい場合、逆シリアル化するSOME / IPネットワークバインディングは、予想されるデータのみを解釈し、予期しないものをスキップします。
」  

スキップされた予期しない部分に続く次の予期されるデータの開始を決定するために、SOME/IPネットワークバインディングは提供された長さ情報を使用できます。
typeフィールドは、要素のタイプを記述します。タイプフィールドの長さは、32、16、8、または0ビットにすることができます。  
